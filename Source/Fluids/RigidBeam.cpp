    ...
    : m_charge(charge)
{
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        positive(mass) && positive(std::abs(charge)),
    ...
    if (energy) {
        ...
        p.m_velocity =
            PhysConst::c * std::sqrt(m_energy_ev * (m_energy_ev + 2 * rest)) / (m_energy_ev + rest);
    } else {
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            positive(std::abs(p.m_velocity)) && std::abs(p.m_velocity) < PhysConst::c,
    ...
    if (transverse_rms) {
        p.m_sigma_r = radial_rms / std::sqrt(2.0);
    }
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        length != time_width, "A rigid beam requires exactly one of sigma_z [m] and sigma_t [s].");
    if (time_width) {
        p.m_sigma_z = std::abs(p.m_velocity) * duration;
    }
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        positive(p.m_sigma_r) && positive(p.m_sigma_z),
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        std::isfinite(p.m_z_reference) && (positive(p.m_cutoff_r) || p.m_cutoff_r == -1.0) &&
            (positive(p.m_cutoff_z) || p.m_cutoff_z == -1.0),
        "Rigid-beam cutoffs must be finite positive sigma multiples, or -1 for no truncation.");
    if (p.m_cutoff_r == -1.0) {
        p.m_cutoff_r = std::numeric_limits<double>::infinity();
    }
    if (p.m_cutoff_z == -1.0) {
        p.m_cutoff_z = std::numeric_limits<double>::infinity();
    }
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(explicit_times != train,
                                     "Specify pulse_times or a finite train with first_pulse_time, "
                                     "pulse_period and pulse_count.");
    if (train) {
    ...
    for (amrex::Long i = 0; i < m_times.size(); ++i) {
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
            std::isfinite(m_times[i]) && std::isfinite(m_amplitudes[i]) && m_amplitudes[i] >= 0.0,
    ...
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(int(by_current) + int(by_charge) + int(by_density) == 1,
                                     "Specify exactly one rigid-beam normalization: peak_current, "
                                     "bunch_charge or peak_density.");
    double const area = p.radialIntegral(0.0, p.m_cutoff_r * p.m_sigma_r);
    if (by_current) {
        p.m_peak_density = current / (std::abs(charge * p.m_velocity) * area);
    }
    if (by_charge) {
        p.m_peak_density =
            bunch_charge /
            (std::abs(charge) * area * p.m_sigma_z *
             warpx::fluid::gaussianIntegral(-p.m_cutoff_z, p.m_cutoff_z, p.m_cutoff_z));
    }
    WARPX_ALWAYS_ASSERT_WITH_MESSAGE(
        positive(p.m_peak_density),
...
RigidBeam::configuration () const
{
    ...
    out << p.m_velocity << ' ' << p.m_sigma_r << ' ' << p.m_sigma_z << ' ' << p.m_peak_density
        << ' ' << p.m_z_reference << ' ' << p.m_cutoff_r << ' ' << p.m_cutoff_z << ' '
        << m_times.size();
    for (amrex::Long i = 0; i < m_times.size(); ++i) {
        out << ' ' << m_times[i] << ' ' << m_amplitudes[i];
    }
    return out.str();
}
...
{
#ifdef WARPX_DIM_RZ
    if (!m_radial.empty() && m_radial_nodal == density.ixType()[0]) {
        return;
    }
        ...
        for (int mirror = 0; mirror < 2; ++mirror) {
            ...
            for (int piece = 0; piece <= order; ++piece) {
                ...
                for (int sub = 0; sub < subdivisions; ++sub) {
                    integral += warpx::fluid::integrate(
                        [=] AMREX_GPU_HOST_DEVICE(double r) noexcept {
                            double const x = r / p.m_sigma_r;
                            return r * std::exp(-0.5 * x * x) *
                                   warpx::fluid::shape(order, (r - center) / dr);
                        },
    ...
    if (m_density_time != time) {
        amrex::ParallelFor(count, [=] AMREX_GPU_DEVICE(int j) noexcept {
            ...
            for (int pulse = first; pulse < last; ++pulse) {
                ...
                value += p.m_amplitudes[pulse] *
                         warpx::fluid::projectedIntegral(order - 1, node - center - dz / 2,
                                                         node - center + dz / 2, dz, p.m_sigma_z,
                                                         p.m_cutoff_z) /
                         dz;
            }
...
void
RigidBeam::DepositCurrent (amrex::MultiFab& current, amrex::Geometry const& geom, amrex::Real start,
                           amrex::Real dt)
{
#ifdef WARPX_DIM_RZ
    if (!active(geom, start, dt, (WarpX::nox + 1) * geom.CellSize(1))) {
        return;
    }
    ...
    if (m_current_time != start || m_current_dt != dt) {
        amrex::ParallelFor(count, [=] AMREX_GPU_DEVICE(int j) noexcept {
            ...
            for (int pulse = first; pulse < last; ++pulse) {
                ...
                if (dt > 0.0) {
                    value += p.m_amplitudes[pulse] * sign *
                             warpx::fluid::projectedIntegral(spectral ? order : order - 1,
                                                             std::min(a, b), std::max(a, b), dz,
                                                             p.m_sigma_z, p.m_cutoff_z) /
                             dt;
                } else {
                    value += p.m_amplitudes[pulse] * p.m_velocity *
                             warpx::fluid::projectedDensity(spectral ? order : order - 1, a, dz,
                                                            p.m_sigma_z, p.m_cutoff_z);
                }
            }
