...
WarpXFluidContainer::PhysicalTotals () const
{
    ...
    amrex::ReduceOps<amrex::ReduceOpSum, amrex::ReduceOpSum, amrex::ReduceOpSum,
                     amrex::ReduceOpSum, amrex::ReduceOpSum> reduce;
    ...
    double const beam_energy = m_rigid_beam ? m_rigid_beam->kineticEnergyEV()*PhysConst::q_e : 0.0;
    double const beam_momentum = species_mass*velocity/std::sqrt(1-velocity*velocity/PhysConst::c2);
    ...
    for (int lev = 0; lev <= warpx.finestLevel(); ++lev) {
        ...
        if (lev < warpx.finestLevel()) {
            fine_mask = amrex::makeFineMask(number, warpx.boxArray(lev+1), warpx.refRatio(lev), 1, 0);
        }
        for (amrex::MFIter mfi(number, amrex::TilingIfNotGPU()); mfi.isValid(); ++mfi) {
            ...
            auto const mask = fine_mask.ok() ? fine_mask.const_array(mfi) : amrex::Array4<int const>{};
            amrex::Array4<amrex::Real const> momentum[3];
            if (model == FluidModel::ColdRelativistic) {
                for (int dir = 0; dir < 3; ++dir) {
                    momentum[dir] = warpx.m_fields.get(name_mf_NU,
                        ablastr::fields::Direction{dir}, lev)->const_array(mfi);
                }
            }
            ...
            reduce.eval(mfi.growntilebox(ng), data,
                [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept -> Tuple {
                    if (!owner(i,j,k) || (mask && !mask(i,j,k))) { return {0,0,0,0,0}; }
                    double volume = AMREX_D_TERM(dx[0], *dx[1], *dx[2]);
#ifdef WARPX_DIM_RZ
                    double const r = lo[0]+(i-domain.smallEnd(0)+0.5*(1-nodal[0]))*dx[0];
                    if (r < 0.0) { return {0,0,0,0,0}; }
                    volume *= r == 0.0 ? MathConst::pi*dx[0]*axis_factor : 2*MathConst::pi*r;
#else
                    ...
                    double const ux = mx(i,j,k)/n(i,j,k), uy = my(i,j,k)/n(i,j,k), uz = mz(i,j,k)/n(i,j,k);
                    double const u2 = ux*ux+uy*uy+uz*uz;
                    double const energy = species_mass*u2/(1+std::sqrt(1+u2/PhysConst::c2));
#ifdef WARPX_DIM_RZ
    ...
    std::array<amrex::Real, 5> totals{amrex::get<0>(value), amrex::get<1>(value),
        amrex::get<2>(value), amrex::get<3>(value), amrex::get<4>(value)};
