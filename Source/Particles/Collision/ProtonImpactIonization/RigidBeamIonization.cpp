...
namespace
{
    ...
    interval (double const* cdf, int count, double value) noexcept
    {
        ...
        while (lo+1 < hi) {
            ...
            if (cdf[mid] <= value) { lo = mid; } else { hi = mid; }
        }
        return lo;
    }
}
...
void
ProtonImpactIonizationCollision::ProduceFromFluid (
    amrex::Real cur_time, amrex::Real start_time, amrex::Real dt, MultiParticleContainer* mypc)
{
...
#ifdef WARPX_DIM_RZ
    if (m_constant_density && m_background_density == 0.0) { return; }
    auto& warpx = WarpX::GetInstance();
    if (!m_fluid_projectile->getRigidBeam()->active(warpx.Geom(0), start_time, dt)) { return; }
    ...
    auto const maximum_transfer = pjg.maximumEnergyTransfer(
        m_fluid_projectile->getRigidBeam()->kineticEnergyEV());
    ...
    for (amrex::MFIter mfi = electron.MakeMFIter(0, info); mfi.isValid(); ++mfi) {
        ...
        amrex::ParallelFor(cells, [=] AMREX_GPU_DEVICE(int cell) noexcept {
            ...
            if (constant_gas) {
                ...
                for (int bin = 0; bin < bins; ++bin) {
                    ...
                    double const background = density_function(
                        rb+0.5*dr/quadrature, 0.0, zb+0.5*dz/quadrature, tb+0.5*dt/quadrature);
            ...
            auto const count = static_cast<amrex::Long>(std::min(double(cap), std::floor(expected)));
            auto weight = expected >= cap ?
                static_cast<amrex::ParticleReal>(available/cap) : fixed_weight;
        ...
        WARPX_ALWAYS_ASSERT_WITH_MESSAGE(first_e+total <= std::numeric_limits<int>::max() &&
            first_i+total <= std::numeric_limits<int>::max(), "Rigid-source product tile overflow.");
        electrons.resize(first_e+total);
        if (ions) { ions->resize(first_i+total); }
        ...
        amrex::ParallelForRNG(cells, [=] AMREX_GPU_DEVICE(
            ...
            for (amrex::Long product = 0; product < count; ++product) {
                ...
                if (constant_gas) {
                    ...
                    z = zlo+dz/quadrature*(b+(target-row[bin])/(row[bin+1]-row[bin]));
                }
                double const r = radialQuantile(r0, r1, profile.m_sigma_r, u_r);
                auto const stored_r = std::min(static_cast<amrex::ParticleReal>(r),
                    std::nextafter(static_cast<amrex::ParticleReal>(rlo+dr),
                                   static_cast<amrex::ParticleReal>(rlo)));
                auto const stored_z = std::min(static_cast<amrex::ParticleReal>(z),
                ...
                if (!fluid_ion) { create_ion(ions_data, pi, engine, stored_r, 0.0, stored_z); }
                auto const theta = 2*MathConst::pi*shiftedKronecker<double>(
                    sample_index, position_shift, 0xa54ff53bu);
                e.m_rdata[PIdx::theta][pe] = static_cast<amrex::ParticleReal>(theta);
                if (!fluid_ion) { ion_data.m_rdata[PIdx::theta][pi] = e.m_rdata[PIdx::theta][pe]; }
                amrex::ParticleReal secondary, threshold;
                pjg.sample(sampling, ProtonImpactIonization::shiftedRadicalInverse(
                    sample_index, energy_shift), secondary, threshold);
                auto const cosine = ProtonImpactIonization::polarCosine(secondary, threshold,
                    maximum_transfer, shiftedKronecker<amrex::ParticleReal>(
                        sample_index, angle_shift, 0x6a09e667u));
                double const phi = 2*MathConst::pi*shiftedKronecker<double>(
                    sample_index, azimuth_shift, 0xbb67ae85u);
                double const speed = BackgroundMCCKinematics::properSpeedFromKineticEnergy(
                    secondary, PhysConst::m_e_v<double>);
                double const transverse = speed*std::sqrt(std::max(0.0, 1-double(cosine)*cosine));
                e.m_rdata[PIdx::ux][pe] = static_cast<amrex::ParticleReal>(transverse*std::cos(phi));
                e.m_rdata[PIdx::uy][pe] = static_cast<amrex::ParticleReal>(transverse*std::sin(phi));
                e.m_rdata[PIdx::uz][pe] = static_cast<amrex::ParticleReal>(
                    speed*cosine*(profile.m_velocity > 0 ? 1 : -1));
                double const kelvin = constant_temperature ? temperature : temperature_function(r, 0, z, cur_time);
                AMREX_ALWAYS_ASSERT(kelvin >= 0.0 && std::isfinite(kelvin));
                double ion_energy = 0.0;
                for (int dir = 0; dir < 3; ++dir) {
                    double const a = shiftedKronecker<double>(sample_index,
                        static_cast<std::uint32_t>(scramble(phase+5+2*dir)), 0x510e527fu);
                    double const b = shiftedKronecker<double>(sample_index,
                        static_cast<std::uint32_t>(scramble(phase+6+2*dir)), 0x7311c281u);
                    double const velocity = kelvin > 0 ? std::sqrt(-2*PhysConst::kb*kelvin/
                        neutral_mass*std::log(a))*std::cos(2*MathConst::pi*b) : 0.0;
                    if (!fluid_ion) { ion_data.m_rdata[PIdx::ux+dir][pi] = static_cast<amrex::ParticleReal>(velocity); }
                    ion_energy += 0.5*ion_mass*velocity*velocity;
                }
                auto const weight = weights[cell];
                e.m_rdata[PIdx::w][pe] = weight;
                if (!fluid_ion) { ion_data.m_rdata[PIdx::w][pi] = weight; }
                emitted += weight;
                energy += weight*double(secondary)*PhysConst::q_e;
                binding += weight*double(threshold)*PhysConst::q_e;
                if (fluid_ion) { discarded += weight*ion_energy; }
            }
            sequence += static_cast<std::uint64_t>(count);
            for (int digit = 0; digit < 4; ++digit) {
                sequences(i,j,0,digit) = static_cast<amrex::Real>((sequence >> (16*digit)) & 65535u);
            }
        ...
        ParticleCreation::DefaultInitializeRuntimeAttributes(electrons, electron,
            static_cast<int>(first_e), static_cast<int>(first_e+total));
        if (ions) {
            ParticleCreation::DefaultInitializeRuntimeAttributes(*ions, *ion,
                static_cast<int>(first_i), static_cast<int>(first_i+total));
        }
        if (fluid_ion) {
            auto const deposit = destination.deposit(0, mfi);
            amrex::For(total, [=] AMREX_GPU_DEVICE(int p) noexcept {
                auto const index = first_e+p;
                deposit(e.m_rdata[PIdx::r][index], e.m_rdata[PIdx::z][index], e.m_rdata[PIdx::w][index]);
            });
        }
        amrex::Gpu::streamSynchronize();
        setNewParticleIDs(electrons, first_e, total);
        if (ions) { setNewParticleIDs(*ions, first_i, total); }
    }
