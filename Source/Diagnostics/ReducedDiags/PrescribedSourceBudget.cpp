...
PrescribedSourceBudget::PrescribedSourceBudget (std::string const& name)
    : ReducedDiags(name)
{
    ...
    for (auto const& collision : collisions) {
        ...
        std::string type;
        pp.query("type", type);
        if (type != "proton_impact_ionization" || !warpx.DoFluidSpecies()) { continue; }
        ...
        if (fluid && fluid->getRigidBeam()) { m_sources.push_back(collision); }
    }
